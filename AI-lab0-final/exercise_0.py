...
def q2(n):
    ...
    '''
    x = balabala
    x=np.arange(n).reshape(n,1)+np.arange(n).reshape(1,n)
    return x
...
def q5():
    ...
    res = balabala
    son=0
    test=np.random.uniform(0,1,(10000,2))
    #testx=np.random.uniform(0,1,10000)
    for i in test:
        if i[0]**2+i[1]**2<1:
            son+=1
    res=son/(10000*0.5*0.5)
            ...
            for j in range(self.width):
                if (i,j) in self.walls:
                    ...
                    if flag==0:
                        ...
                        right=j
                elif (i,j)==self.pacman_pos:
                    row+='P'
                elif [i,j] in self.foods:
                    row+='.'
                else:
                    row+=' '
            str+=row[left:right+1]
            str+='\n'
        return str

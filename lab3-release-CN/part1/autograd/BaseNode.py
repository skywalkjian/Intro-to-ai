...
class Attention(Node):
    ...
    def cal(self, X):
        ...
        #print(f"X dtype: {X.dtype}, shape: {X.shape}, max: {np.max(X):.2e}, min: {np.min(X):.2e}, mean: {np.mean(X):.2e}, std: {np.std(X):.2e}")
        #print(f"W_q dtype: {W_q.dtype}, shape: {W_q.shape}, max: {np.max(W_q):.2e}, min: {np.min(W_q):.2e}, mean: {np.mean(W_q):.2e}, std: {np.std(W_q):.2e}")
        #print(f"W_k dtype: {W_k.dtype}, shape: {W_k.shape}, max: {np.max(W_k):.2e}, min: {np.min(W_k):.2e}, mean: {np.mean(W_k):.2e}, std: {np.std(W_k):.2e}")
        Q = (X @ W_q).astype(np.float64)  # (B,L,d)
        K = (X @ W_k).astype(np.float64)  # (B,L,d) 
        V = (X @ W_v).astype(np.float64)  # (B,L,d)
        #print(f"Q dtype: {Q.dtype}, shape: {Q.shape}, max: {np.max(Q):.2e}, min: {np.min(Q):.2e}, mean: {np.mean(Q):.2e}, std: {np.std(Q):.2e}")
        #print(f"K dtype: {K.dtype}, shape: {K.shape}, max: {np.max(K):.2e}, min: {np.min(K):.2e}, mean: {np.mean(K):.2e}, std: {np.std(K):.2e}")
        scores = Q @ (K.transpose(0,2,1)/np.sqrt(self.dim))  # (B,L,L)
        #print(f" scores 是否包含 NaN: {np.isnan(scores).any()}")
        #print(f" scores 是否包含 Inf: {np.isinf(scores).any()}")
        #if np.isnan(scores).any() or np.isinf(scores).any():
         #   print("警告：在 Softmax 之前，scores 数组已包含 NaN 或 Inf。")
    # 可以打印部分 scores 来查看具体数值
       # print("部分 scores 示例:\n", scores[:5, :5])
        
        softmax_score = np.exp(scores - scores.max(axis=-1, keepdims=True))
        softmax_score = softmax_score / (softmax_score.sum(axis=-1, keepdims=True) + 1e-8)
        out = softmax_score @ V  # (B,L,d)
...
class Linear(Node):
    ...
    def cal(self, X):
        # TODO: YOUR CODE HERE
        self.cache.append(X)
        return X @ self.params[0] + self.params[1].reshape(1, -1)
...
class ResLinear(Node):
    ...
    def cal(self, X):
        ...
        # 即Linear(X)+X
        self.cache.append(X)
        return X @ self.params[0] + self.params[1].reshape(1, -1) + X
    ...
    def backcal(self, grad):
        # TODO: YOUR CODE HERE
        X = self.cache[-1]
        self.grad.append((X.reshape(-1, X.shape[-1])).T @ grad.reshape(-1, grad.shape[-1]))
        self.grad.append(grad.reshape(-1, grad.shape[-1]).sum(0))

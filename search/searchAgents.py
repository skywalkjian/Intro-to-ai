...
def cornersHeuristic(state, problem):
    ...
    h_sum=util.manhattanDistance( node, unv[0] )
    tarpos=unv[0]
  
    for i in unv:
        if h_sum>util.manhattanDistance(node,i):
            h_sum=util.manhattanDistance(node,i)
            tarpos=i
    unv.remove(tarpos)
    fornum=len(unv)
    for i in range(fornum):
        num=util.manhattanDistance(tarpos,unv[0])
        tarpos2=unv[0]
        for j in unv:
            if num>util.manhattanDistance(tarpos,j):
                num=util.manhattanDistance(tarpos,j)
                tarpos2=unv[0]
